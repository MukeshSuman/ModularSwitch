...
const SwitchboardItem: React.FC<SwitchboardItemProps> = ({ label, isOn, size, onToggle, onRemove }) => (
  <div className={`bg-white shadow-md flex flex-row items-center relative ${size === 'full' ? 'w-full h-full' : 'w-1/2 h-full'}`}>
    <button 
...
interface Box {
  ...
  x: number;
  y: number;
  items: { label: string; size: 'full' | 'half'; isOn: boolean }[];
}
...
const TailwindModalBoxSystem: React.FC = () => {
  ...
  const addItemToBox = (item: { label: string; size: 'full' | 'half' }) => {
